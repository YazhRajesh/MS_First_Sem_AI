...
//IDA Star Algorithm:
// initialize minimum (initial node's depth) to be the cutoff (of initial node)
...
// IDA* does not utilize dynamic programming and therefore often ends up exploring the same nodes many times.
...
public class idastar 
{
		...
		{   
			...
			{
				 System.out.println("\nIDA* Heuristic 1 (Misplaced Tiles):");
					String puzzle = "";
					node root= new node();
					root.change= "Start";
				...
				int bound= getManhattanSum(root1);     //to find the bound of the root 
				initializer astarh2sol = astarh2 (root1, puzzle1, bound);  //call the function to implement
				if(astarh2sol !=null)     //if solution found print the output
					{
					System.out.println("IDAStar H2: ");
		...
		public static initializer astarh1(node root, String puzzle )  //Misplaced Tiles: Astar Heuristic 1 Implementation 
		{
			LinkedList<node> unexpanded = new LinkedList<node>();
	        unexpanded.add(root);
	       // int depth=0;
	        ...
	        while(expanded < Integer.MAX_VALUE)
	        {
	            ...
	            {
	                node tmp = unexpanded.remove(i);
	                if(findTilesMissplaced(tmp) + tmp.level < minimum)
	                {
	                    minimum = findTilesMissplaced(tmp) + tmp.level;  //minimum is the cutoff
	                    currentValue = tmp;
	                 //  System.out.println("cutoff" + minimum);
	                }
	            }
	            ...
	            else if(currentValue.level <=minimum)   //f(N) <= Cutoff
	            {  // System.out.println("f(n)" + currentValue.level);
		...
		public static initializer astarh2(node root, String puzzle, int bound)  //Manhattan Sum: AStar Heuristic 2 Implementation 
		{   
			LinkedList<node> unexpanded = new LinkedList<node>();
	        unexpanded.add(root);
	        ...
	        while(expanded < Integer.MAX_VALUE)
	        {
	             ...
	             for (int i = 0; i < unexpanded.size() ; i++) 
	             {
	                 node temporary = unexpanded.remove(i);
	                 if(getManhattanSum(temporary) + temporary.level < minimum)  
	                 {
	                     minimum = getManhattanSum(temporary) + temporary.level;  //cutoff value
	            ...
	            if (findTilesMissplaced(currentValue) == 0)  //if output- print output
	            {
